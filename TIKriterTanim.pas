...
uses
  ...
  KadirLabel,data_modul, cxTextEdit, Vcl.Menus, cxMemo;

type
  TfrmTIKriter = class(TGirisForm)
    ...
    Grid_Sks_TanimDBTableView1Column4: TcxGridDBColumn;
    Grid_Sks_TanimDBTableView1Column5: TcxGridDBColumn;
    Grid_Sks_TanimDBTableView1Column6: TcxGridDBColumn;
...
var
  frmTIKriter: TfrmTIKriter;

implementation

{$R *.dfm}

procedure TfrmTIKriter.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 Grid_Sks_Tanim.Dataset.Active := False;
end;

procedure TfrmTIKriter.FormCreate(Sender: TObject);
begin
  //Menu := PopupMenu1;
  Grid_Sks_Tanim.Dataset.Active := False;
  Grid_Sks_Tanim.Dataset.Connection := datalar.ADOConnection2;
  Grid_Sks_Tanim.Dataset.SQL.Text := 'select * from istatistik_Tanim_Tablosu where Tanim = ''TIBBIDENETIM''';
  Grid_Sks_Tanim.Dataset.Active := True;
  cxPanel.Visible := False;
end;

procedure TfrmTIKriter.G1Click(Sender: TObject);
var
...
procedure TfrmTIKriter.Grid_Sks_TanimDBTableView1NavigatorButtonsButtonClick(
  Sender: TObject; AButtonIndex: Integer; var ADone: Boolean);
begin
  inherited;
   (*
