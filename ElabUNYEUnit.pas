...
uses
  ...
  Dialogs, StdCtrls, Grids, BaseGrid,ComCtrls, Mask,sGauge,ElabUNYE,data_modul,DateUtils;
  ...
  procedure SonucAl(t1,t2 : TDateTime ;gridAktif : TcxGridDBTableView ; Log : Tcxmemo ; progres :TcxProgressBar);
        ...
        Then Begin
           ...
           id := gridAktif.DataController.GetValue(
                 ...
                 Then begin
                     ...
                     try
                      lvGetTestResult := (datalar.Lab as ElabUNYE.eLabPortType).AddNewPatientRecord(lvGetTestResultWithDateRangeRequest);
...
var
  lvRIO: THTTPRIO;
  lvGetTestResultWithDateRangeRequest : GetTestResultWithTCKNoRequest;//GetTestResultWithDateRangeRequest;
  lvGetTestResult : GetTestResultWithTCKNoResponse;
  tcs : MobilePhoneList;
  i : integer;
  min,max,_F_ ,testKod ,sql ,dosyaNo , gelisNo , _tc_ ,id ,kayittip : string;
 durum : boolean;
...
begin
  lvGetTestResultWithDateRangeRequest := GetTestResultWithTCKNoRequest.Create;

  lvGetTestResultWithDateRangeRequest.Login := Login.Create;
  lvGetTestResultWithDateRangeRequest.Login.UserName := datalar._labusername; //'user';
  lvGetTestResultWithDateRangeRequest.Login.PassWord := datalar._labsifre; //'12345';
  lvGetTestResultWithDateRangeRequest.StartTime  := TXSDateTime.Create;
  lvGetTestResultWithDateRangeRequest.StartTime.XSToNative(FormatDateTime('YYYY-MM-DD',t1)+'T00:00:00');
  lvGetTestResultWithDateRangeRequest.EndTime  := TXSDateTime.Create;
      lvGetTestResultWithDateRangeRequest.EndTime.XSToNative(FormatDateTime('YYYY-MM-DD',t2)+'T00:00:00');
  datalar.Lab.URL := datalar._laburl;


  if not DirectoryExists('C:\NoktaV3\UNYE')
  then
   MkDir('C:\NoktaV3\UNYE');
   ...
   begin
       ...
       kayitTip := varToStr(gridAktif.DataController.GetValue(
        ...
        Then Begin
           ...
           lvGetTestResultWithDateRangeRequest.TCKNo := tcs;


           datalar.HTTP_XMLDosya_Name := 'C:\NoktaV3\UNYE\ELABSonuc_' + dosyaNo + '_' + gelisNo;

           lvGetTestResult := (datalar.Lab as ElabUNYE.eLabPortType).GetTestResultWithTCKNo(lvGetTestResultWithDateRangeRequest);


           SonucYaz(lvGetTestResult.GetTestResultWithTCKNoResult,0);
...
var
  ...
  durum : boolean;
  barkods : MobilePhoneList;  //Array_Of_string;
   ...
   begin
       ...
       kayitTip := varToStr(gridAktif.DataController.GetValue(
        ...
        //   StringArray := TArray<string>.Create(barkodG,barkodC,OrnekNo_TamKan);
           ...
           try
             lvGetTestResult := (datalar.Lab as ElabUNYE.eLabPortType).GetTestResultWithBarcodeNo(lvGetTestResultWithDateRangeRequest);
