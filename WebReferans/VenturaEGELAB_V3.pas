...
uses
  ...
  ExtCtrls, Buttons, sBitBtn, AdvToolBtn, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit, cxTextEdit,
  IHastaEkle1, ITESTEKLE1,ISonucAl1, Winsoftservice,
  //IMDLSHastaIslemleri1,TenayService1,LabIslemleri,Bridge_delphi1, AlisWs,AEN1,
  //Referans,BirlabService,Duzen,data_model1,alisHizmetler,IMeddataLabService2,interTIP,
  ...
  AdvObj, sButton, DB, dxmdaset, ImgList, cxGroupBox, cxRadioGroup, DBClient,
  //LABserviceERBIL,AlisHizmetlerV101,ENA,NoktaFormUnit,TenayserviceV2, LabratuarIstek,
  Provider, xmldom, XMLIntf, msxmldom, Xmlxform,
  DBGrids,comObj,dateutils, acProgressBar, cxProgressBar,
  cxButtons, cxLabel, cxSpinEdit,SuperObject,IdHTTP, IdBaseComponent,
  IdComponent, IdTCPConnection, IdTCPClient, SQLMemMain,
  VenturaServiceEGELAB;
...
type
 ...
 //   function OrderMNT(dosyaNo : string ; gelis : string) : TenayServiceV4.Order;
  //  function OrderBIO(dosyaNo : string ; gelis : string) : TenayServiceBio.Order;
    ...
    function HastaIstekBilgileriMetdata(dosyaNo : string ; gelisNo : string ; tarih : string;tip : string) : IMeddataLabService2.Array_of_Tistekler;
...
//    procedure TenaySonucAlBio(_dosyaNo,_gelisNo,_RefId : string);
    ...
    function HastabilgileriDoldurBirlab(dosyaNo : string ; gelis : string ; id : integer) : ISuperObject;
    function HastaIstekBilgileriBirlab(dosyaNo : string ; gelisNo : string) : ISuperObject;
    ...
    procedure S19Click(Sender: TObject);
...
uses rapor , ReferansList , BioLab , MntLab, muhana,TenayV4,Elab;
...
begin
   sql :=
          'select g.dosyaNo,g.gelisNo,g.BHDAT ,ornekNo,CikisOrnekNo,' +
          ...
          ',OrnekNo_Plazma,OrnekNo_Serum,OrnekNo_TamKan,LabaratuvarKabulZamani,UzmanOnayZamani ' +
          ',case when isnull(g.KanAlimZamani,'''') = '''' then RTarih else g.KanAlimZamani end KanalimZamani ' +
          ' from gelisler g ' +
          ' left join gelisdetay gd on gd.dosyaNo = g.dosyaNO and gd.gelisNo = g.gelisNo and KanAlindimi = 1 ' +
   ...
   begin
       ...
       gridAktif.Cells[13,x] := datalar.ADO_SQL.fieldbyname('KanAlimZamani').AsString;
       gridAktif.Cells[14,x] := datalar.ADO_SQL.fieldbyname('LabaratuvarKabulZamani').AsString;
       gridAktif.Cells[15,x] := datalar.ADO_SQL.fieldbyname('UzmanOnayZamani').AsString;
...
var
 x : integer;
 ado : TADOQuery;
 sql , kanalimZamani,kabulZamani,onayZamani: string;
begin
    ado := TADOQuery.Create(nil);
    ado.Connection := datalar.ADOConnection2;
    ...
    begin
      ...
      try
        try
         kanalimZamani := FormatDateTime('YYYY-MM-DD HH:MM',strtodatetime(gridAktif.cells[13,x]));
         kabulzamani :=  gridAktif.cells[14,x];
         kabulzamani := FormatDateTime('YYYY-MM-DD HH:MM',strtodatetime(kabulZamani));
         onayZamani := FormatDateTime('YYYY-MM-DD HH:MM',strtodatetime(gridAktif.cells[15,x]));
        except
        end;
       ...
       sql := 'update gelisler set OrnekNo = ' + QuotedStr(gridAktif.Cells[4,x]) +  ', CikisOrnekNO = ' + QuotedStr(gridAktif.Cells[5,x]) +
              ...
              ',KanAlimZamani = ' + QuotedStr(kanalimZamani) +
              ',LabaratuvarKabulZamani = ' + ifThen(kabulZamani = '','NULL', QuotedStr(kabulZamani)) +
              ',UzmanOnayZamani = ' + ifThen(onayZamani = '','NULL', QuotedStr(onayZamani))  +
              ',LabRefId = ' +  QuotedStr(gridAktif.Cells[9,x]) +
       ...
       datalar.QueryExec(ado,sql);
        ...
        (*
         sql := 'update Centro_barkodlar set durum = 1 ' +
                ' where barkodNo in (' + QuotedStr(gridAktif.Cells[4,x]) + ',' + QuotedStr(gridAktif.Cells[5,x]) + ','

                                       + QuotedStr(gridAktif.Cells[12,x]) + ')';
...
begin
 ...
 try
   ...
   begin
       ...
       _F_ := '';
       testKod := KodEslestir(testid,_F_);
       if tip = 'Ç' then _F_ := 'cd' else _F_ := 'gd';
...
procedure TfrmLabOrnekNoGiris.B3Click(Sender: TObject);
var
  i , j , r : integer;
  p : TPoint;
  dosyaNo,gelisNo,sql,GC,ss,ornekNo,id,_F_,testKod,min,max,sonuc,sonucA,sm,barkod : string;
  ado,adoB : TADOQuery;
  t : boolean;
...
begin
    ...
    Begin
      ...
      if  (t = True) and (gridAktif.Cells[4,r] = '') and (gridAktif.Cells[5,r] = '') and (gridAktif.Cells[12,r] = '')
          and (gridAktif.Cells[8,r] = 'Yeni Kayýt')
      Then Begin
         ...
         sql := 'select top 1 barkodNo from Centro_barkodlar where durum = 0';
         datalar.QuerySelect(adoB,sql);
         i := 0;
         barkod := adoB.Fields[0].AsString;
         (*
         ...
         begin
              gridAktif.Cells[ado.FieldByName('colNo').AsInteger,r] := barkod + ado.FieldByName('grupKodu').AsString;
              ado.Next;
              i := i + 1;
         end;
             *)

         gridAktif.Cells[4,r] := barkod; // giris
         gridAktif.ints[5,r] := strToint(barkod) + 1; // Cýkýs
         gridAktif.ints[12,r] := strToint(barkod) + 2;  // tam kan
         gridAktif.ints[10,r] := strToint(barkod) + 4;  // plazma
         gridAktif.ints[11,r] := strToint(barkod) + 3;  // biakarbonat
         ...
         sql := 'update Centro_barkodlar set durum = 1,dosyaNo = ' + QuotedStr(dosyaNo) + ',gelisNo = ' + QuotedStr(gelisNo) +
                ' where barkodNo = ' + QuotedStr(barkod);
         datalar.QueryExec(sql);
...
procedure TfrmLabOrnekNoGiris.gridAktifGetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
begin
  inherited;
    if (acol = 13) or (acol = 14) or (acol = 15)
    then aEditor := edDateTimeEdit;
end;
...
begin
  ...
  begin
       case gridAktif.Col of
        14,15 : gridAktif.cells[gridAktif.Col,gridAktif.Row] := '';
       end;
...
procedure TfrmLabOrnekNoGiris.H10Click(Sender: TObject);
var
...
begin
   ...
   begin
     ...
     Then Begin
         ...
         dosyaNo := gridAktif.Cells[1,I];
         gelisNo := gridAktif.Cells[2,I];
         adi := gridAktif.Cells[3,I];
         gelisTarihi := gridAktif.Cells[21,I];
        ...
        try
            ...
            url := datalar._laburl;
           // Json := HastabilgileriDoldurBirlab(dosyaNo,gelisNo,strtoint(id));
            sql := 'select * from HastaKart where dosyano = ' + QuotedStr(dosyaNo);
            datalar.QuerySelect(ado1,sql);

            Json := SO;
           // JSon.S['Tarih'] := '2016-10-10'; // String Alan
            JSon.S['Adi'] := ado1.fieldbyname('HASTAADI').AsString;
            JSon.S['Soyadi'] := ado1.fieldbyname('HASTASOYADI').AsString;
            JSon.S['TCNo'] := ado1.fieldbyname('TCKIMLIKNO').AsString;
            JSon.S['Cinsiyet'] := ifThen(ado1.fieldbyname('CINSIYETI').AsString = '0','E','K');
            JSon.S['DogumTarihi'] := FormattedTarih(ado1.fieldbyname('DOGUMTARIHI').Asstring);
           ...
           // JSon.O['Testler'] := HastaIstekBilgileriBirlab(dosyaNo,gelisNo).O['Testler'];
            sql := 'select l.islemKodu,''G'' Tip,h.SIRANO from hareketler h' +
                   ...
                   ' Union All ' +
                   'select l.islemKoduC,''C'' Tip,h.SIRANO from hareketler h' +
                   ' join labtestler l on l.butKodu = h.code ' +
                   ' where dosyaNo = ' + QuotedStr(dosyaNo) + ' and onay = 1 and gelisNo = ' + gelisNo +
                   ' and charindex(''.'',code) = 0  and l.tip = 2 and uygulamaAdet = 2';
            ...
            begin
              SutSon := SO;
              SutSon.S['SutKodu'] := ado1.fieldbyname('islemKodu').AsString;
              SutSon.I['IslemId'] := ado1.fieldbyname('SIRANO').AsInteger;
            ...
            Request.Position := 0;
            idHttp1.Post(url+'/kayit', Request, Response);

            Request.SaveToFile('Birlab.txt');
            Response.SaveToFile('BirlabSendCvp.txt');
            try
...
procedure TfrmLabOrnekNoGiris.H11Click(Sender: TObject);
begin
  inherited;

  TenayOrderKaydetENA(gridAktif,txtLog,Progres);
...
begin
  ...
  if (datalar._labfirma = 'MNT') or (datalar._labfirma = 'TenayV4')
  then begin
...
begin
  ...
  End;// length(alisCvp.Patients) > 0
   ...
   txtLog.Lines.Add(alisCvp.SonucMesaj + ' ' + alisCvp.UyariMesaj);
  ...
  Progres.MaxValue := gridAktif.RowCount - 1;
   ...
   begin
     ...
     Then Begin
        ...
        Then Begin
          sonucYaz;
        End else txtLog.Lines.Add(inttostr(alisGon.OrnekNo) + ' - ' + alisCvp.SonucMesaj);
...
function TfrmLabOrnekNoGiris.HastaIstekBilgileriMetdata(dosyaNo : string ; gelisNo : string ; tarih : string;tip : string) : IMeddataLabService2.Array_of_Tistekler;
var
  sql : string;
  istek : Tistekler;
...
procedure TfrmLabOrnekNoGiris.i1Click(Sender: TObject);
var
  Sql , VENPASS,VENUSER,KURUM_KODU , SERVIS_KODU ,ss ,sm, dosyaNo,tc,gelisNo,islemNo : string;
  testKod ,test , min,max, _F_,sonuc ,sonucA ,co,go,id ,tarih1,tarih2: string;
  ado : TADOQuery;
  x, testAdet : integer;
  t : boolean;
  url,_url_ : string;
...
begin
   ...
   begin
     ...
     Then Begin
         ...
         interTIP.SonucAl(SList,KURUM_KODU,VENUSER,go);
         SList.Clear;
         interTIP.SonucAl(SList,KURUM_KODU,VENUSER,co);
         SList.Free;
     End;
   end;
...
var
  sql,kadi,sifre,dosyaNo,gelisNo,id,go,co ,sm,_F_,testKod,t1,t2 : string;
  I : integer;
  t : boolean;
  sonuclar1 : TLabsonuc;
  tc : double;
...
begin
      ...
      begin
        ...
        begin
          testKod := KodEslestir(floattostr(sonuclar.labsonuc[y].sonuclar[x].ISLEM_KODU),_F_);

          case TMenuItem(sender).Tag of
            104 : begin
                  if pos('Çýkýþ',sonuclar.labsonuc[y].sonuclar[x].ISLEM_ADI) > 0
                  Then _F_ := 'Cd'
                  Else
                  _F_ := 'Gd';
            end;
          ...
          Then Begin
             ...
             try
               ...
               if (pos('NEG',sonuclar.labsonuc[y].sonuclar[x].Sonuc) > 0) or
                  (pos('POZ',sonuclar.labsonuc[y].sonuclar[x].Sonuc) > 0)
               then begin
                 sql := 'update hareketler set islemAciklamasi  = ' +
                 QuotedStr(
                  StringReplace(StringReplace(sonuclar.labsonuc[y].sonuclar[x].SONUC,'Negatif','',[rfReplaceAll]),'Pozitif','',[rfReplaceAll])
                 ) +
                        ' where onay = 1 and code = ' + QuotedStr(testKod) + ' and dosyaNo = ' +
                         QuotedStr(dosyaNo) + ' and gelisNO = ' + gelisNo ;
                 datalar.QueryExec(ado,sql);
        ...
        case TMenuItem(sender).Tag of
         ...
         103 :
           Sonuclar1 := (datalar.metdata as IMeddataLabService).labsonuc('','',Encode64(datalar._labusername),
                                                                      ...
                                                                      strtoint(datalar._labkurumkod));
         104 : begin
           Sonuclar1 := (datalar.metdata as IMeddataLabService).labsonucTC_NO(Encode64(datalar._labusername),
                                                                              Encode64(datalar._labsifre),
                                                                              t1,
                                                                              t2,tc,
                                                                              strtoint(datalar._labkurumkod));
...
procedure TfrmLabOrnekNoGiris.S19Click(Sender: TObject);
begin
  inherited;
  TenaySonucAlTCdenENA('','',tarihal(txtTarih.Date),tarihal(txttarih1.Date),gridAktif,txtLog,progres,ref.Checked);
end;
...
begin
    ...
    begin
     ...
     //   b := HTSOCvp.Testler[x].NormalDeger;
         ...
         sonucA := StringReplace(HTSOCvp.Testler[x].Aciklama,'Neg','NEG',[rfReplaceAll]);
         sonucA := StringReplace(HTSOCvp.Testler[x].Aciklama,'Poz','POZ',[rfReplaceAll]);
        ...
        try
           ...
           if (testKod = '907440') or
              ...
              (testKod = '906660')
           Then Begin
              sonucA := trim(StringReplace(StringReplace(HTSOCvp.Testler[x].Sonuc,'>','',[rfReplaceAll]),'<','',[rfReplaceAll]));
              sonucA := trim(StringReplace(StringReplace(SonucA,'NEGatif','',[rfReplaceAll]),'POZitif','',[rfReplaceAll]));
              sonucA := trim(StringReplace(StringReplace(SonucA,'NEGATÝF','',[rfReplaceAll]),'POZÝTÝF','',[rfReplaceAll]));
              sonucA := trim(StringReplace(SonucA,'POZITIF','',[rfReplaceAll]));
...
begin
  datalar.Login;
 (*
  if datalar._labfirma = 'TenayV4'
  then  begin
    Tenayv4.SonucTc(tarihal(txtTarih.date),tarihal(txttarih1.Date), gridAktif,txtLog,progres);
  end;
   *)
  if (datalar._labfirma = 'MNT') or (datalar._labfirma = 'TenayV4')
  then  begin
    ...
    then
       ...
       TenaySonucAlMNTTC('','',gridAktif,txtLog,progres,tarihal(txtTarih.date),tarihal(txttarih1.Date));
   ...
   begin
     ...
     Then Begin
        ...
        try
          Response.Clear;
          url := datalar._laburl;
          idHttp1.Get(url + '/sonuc/'+tc+'/'+tarih1+'/'+tarih2+'', Response);
...
procedure TfrmLabOrnekNoGiris.E3Click(Sender: TObject);
var
 t1,t2 : TXSDateTime;
begin
  t1 := TXSDateTime.Create;
  ...
  txtinfo.Caption := 'Sonuçlar Servisten Okunuyor , Lütfen Bekleyiniz...';
  Application.ProcessMessages;
  Elab.SonucAl(t1,t2,gridAktif,txtLog);

  txtinfo.Caption := 'Sonuçlar Servisten Okundu.';
  Application.ProcessMessages;
